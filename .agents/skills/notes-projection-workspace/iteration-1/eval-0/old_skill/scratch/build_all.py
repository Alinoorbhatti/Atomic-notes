...
# Delete existing directory if it exists to start fresh
if os.path.exists(topic_dir):
    shutil.rmtree(topic_dir)

index_file_path = os.path.join(vault_path, f"{topic_slug}-Index.md")
...
cd_dir = os.path.join(topic_dir, "cross-domain")
...
concepts_data = [
  ...
  },
  {
    ...
    "sources": [
      ...
      {"url": "https://www.thelancet.com/journals/transcript-file", "title": "Clinical Expectancy Studies"}
    ],
    "wikilink_targets": [
...
cross_domain_data = [
    {
        "title": "Sociology", 
        "desc": "Explores how group expectations and structural positions generate self-fulfilling prophecies.",
        "conns": ["Self-Fulfilling Prophecy", "Social Identity Theory Connection"]
    },
    {
        "title": "Educational Psychology", 
        "desc": "Focuses on teacher behavior, curriculum design, and student motivation.",
        "conns": ["Robert Rosenthal", "Lenore Jacobson", "Oak School Study"]
    },
    {
        "title": "Organizational Behavior", 
        "desc": "Studies leadership styles, employee engagement, and corporate culture.",
        "conns": ["Pygmalion in Management", "The Golem Effect", "Pygmalion Effect Mitigation"]
    },
    {
        "title": "Sports Psychology", 
        "desc": "Investigates athletic motivation, coach-athlete relationships, and performance anxiety.",
        "conns": ["Sports and Coaching Expectations", "The Galatea Effect", "The Pygmalion Cycle"]
    },
    {
        "title": "Behavioral Economics", 
        "desc": "Examines how market beliefs and expectations drive economic performance and bubbles.",
        "conns": ["Self-Fulfilling Prophecy", "Expectancy Bias", "The Pygmalion Cycle"]
    },
    {
        "title": "Clinical Psychology", 
        "desc": "Studies patient-therapist expectations and their role in therapeutic outcomes.",
        "conns": ["Medical and Nursing Care", "Interpersonal Expectancy Effects", "Expectancy Bias"]
    }
]
...
# 1. Write the research files
for data in concepts_data:
    r_file = os.path.join(research_dir, f"{data['slug']}.md")
    content = [
        f"CONCEPT: {data['title']}",
        f"SLUG: {data['slug']}",
    ...
    for src in data['sources']:
        content.append(f"- {src['url']} | {src['title']} | accessed 2026-07-12")
    content.append("")
    content.append("WIKILINK_TARGETS:")
    for target in data['wikilink_targets']:
        content.append(f"- {target['name']}")
    content.append("")
    content.append("CROSS_DOMAIN_TARGETS:")
    for cd in data['cross_domain_targets']:
        content.append(f"- {cd['name']}")
    content.append("")
    content.append("DIAGRAM:")
    content.append("<mermaid>")
    content.append(data['diagram'])
    content.append("</mermaid>")
    content.append("")
    content.append("KEY_FIGURES_MENTIONED:")
    for fig in data['key_figures_mentioned']:
        content.append(f"- {fig['name']}: {fig['role']}")
    content.append("")
    content.append("FURTHER_READING:")
    for fr in data['further_reading']:
        content.append(f"- {fr['resource']}: {fr['why']}")
    content.append("")
    content.append("RESEARCH_DONE")
    
    with open(r_file, "w") as f:
        f.write("\n".join(content))

# 2. Write the cross-domain notes (with resolved links so they aren't weak)
for cd in cross_domain_data:
    cd_slug = cd['title'].lower().replace(" ", "-")
    cd_file = os.path.join(cd_dir, f"{cd_slug}.md")
    
    lines = [
        "---",
        f"title: \"{cd['title']}\"",
        "topic: \"The Pygmalion Effect\"",
        "tags: [the-pygmalion-effect, cross-domain]",
        "status: seedling",
        "created: 2026-07-12",
        "type: note",
        "---",
    ...
    for conn in cd['conns']:
        lines.append(f"- [[{conn}]] — Related concept in The Pygmalion Effect.")
    lines.append("")
    lines.append("---")
    lines.append("*Part of [[The Pygmalion Effect-Index]]*")
    
    with open(cd_file, "w") as f:
        f.write("\n".join(lines))

# 3. Write subtopic notes
for st in subtopics_data:
    ...
    lines = [
        ...
        "topic: \"The Pygmalion Effect\"",
        f"bucket: \"{st['bucket']}\"",
        f"tags: [the-pygmalion-effect, {st['bucket_slug']}, subtopic]",
        "status: seedling",
        "created: 2026-07-12",
        "type: note",
        "---",
...
for data in concepts_data:
    ...
    fm = [
        ...
        f"tags: [the-pygmalion-effect, {data['bucket_slug']}, {data['slug']}]",
        "aliases: []",
        "status: seedling",
        "created: 2026-07-12",
        "type: note"
...
moc_configs = [
    {
        ...
        "title": "Foundations",
        "desc": "The history, seminal studies, and theoretical origins of the phenomenon.",
        "content": """## Start here
...
- [[Pygmalion in the Classroom]] — The published book that detailed the study and sparked national debate on educational equity."""
    ...
    {
        "filename": "mechanisms.md",
        "title": "Mechanisms",
        "desc": "The behavioral, cognitive, and communicative processes that drive the effect.",
        "content": """## Start here
...
- [[Feedback Factor]] — The qualitative difference in corrections and praise offered to individuals."""
    ...
    {
        "filename": "applications-and-critiques.md",
        "title": "Applications and Critiques",
        "desc": "Real-world implementations, negative counterparts, replication status, and ethical limits.",
        "content": """## Start here
...
for mc in moc_configs:
    moc_file = os.path.join(mocs_dir, mc['filename'])
    lines = [
        "---",
        f"title: \"{mc['title']}\"",
        "topic: \"The Pygmalion Effect\"",
        f"bucket: \"{mc['title']}\"",
        f"tags: [the-pygmalion-effect, {mc['filename'].replace('.md', '')}, moc]",
        "created: 2026-07-12",
        "type: note",
        "---",
        "",
...
# 6. Write master index
concepts_info = [{"title": d['title'], "bucket": d['bucket'], "sources": len(d['sources'])} for d in concepts_data]
index_lines = [
    "---",
    "title: \"The Pygmalion Effect-Index\"",
    "topic: \"The Pygmalion Effect\"",
...
with open(os.path.join(topic_dir, "_Glossary.md"), "w") as f:
    f.write("\n".join(glossary_lines))

print("All files generated. Starting link audit...")
